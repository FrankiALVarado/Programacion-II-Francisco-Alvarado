...
void cArbol::akinator(cNodo & nodo)
{
	...
	if (eleccion != "s" && eleccion != "n" && eleccion != "x") {
		...
		node = crearCategoria(); //crear categoria
		insertarNodo(*node, nodo);
		//nodo.m_hijoIzq = node;

		cNodo *miembro = new cNodo;
		miembro = crearMiembro(); 
		insertarNodo(*miembro, *node);
	...
	else if (eleccion == "n" && !nodo.m_categoria && nodo.m_hijoDer != nullptr) { // si no atina y todavía hay miembros de la categoria
...
void cArbol::insertarNodo(cNodo & nodo, cNodo & padre)
{
	nodo.m_categoria ? padre.m_hijoIzq = &nodo : padre.m_hijoDer = &nodo;
}
...
cNodo * cArbol::crearCategoria(const string & pregunta)
{
	cNodo * nodo = new cNodo;
	nodo->inicializarNodo(*nodo, pregunta);
	return nodo;
...
cNodo * cArbol::crearMiembro(const string & pregunta)
{
	cNodo * nodo = new cNodo;
	nodo->inicializarMiembro(*nodo, pregunta);
	return nodo;
...
void cArbol::crearHijosRaiz(cNodo & padre)
{
	...
	nodo = crearCategoria("Personaje de tv");
	nodo2 = crearCategoria("Animales");
...
void cArbol::crearMiembrosHijos(cNodo & izq, cNodo & der)
{
	...
	nodo = crearMiembro("Homero Simpson");
	nodo2 = crearMiembro("Cocodrilo");
